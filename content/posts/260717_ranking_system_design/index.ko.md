...
description = "이벤트 기반 일별 상품 랭킹 시스템을 설계하면서 Redis 실시간 랭킹 구조와 RDB Metric SOT 구조를 비교해본 기록"
summary = "Redis 기반 실시간 랭킹 구조에서 시작해, 복구 가능성과 기간별 랭킹 확장을 고려한 RDB Metric SOT + Redis Top N 구조까지 설계를 고도화해본 기록"
...
이번에는 사용자 행동 이벤트를 기반으로 일별 상품 랭킹 시스템을 설계해보았다.

처음 생각한 설계 방식은 상품 조회, 좋아요, 결제 성공 이벤트를 Kafka로 발행하고, `commerce-streamer`가 이벤트를 소비하면서 Redis Sorted Set에 랭킹 점수를 실시간으로 반영하는 방식이다. Redis는 Top N 조회와 특정 상품 rank 조회를 빠르게 처리할 수 있기 때문에 실시간 랭킹의 serving store로 잘 맞았다.
...
판매는 `가격 * 수량`을 기반으로 하되 로그를 적용했다. 판매 금액을 그대로 사용하면 특정 고가 상품이나 인기 상품이 한 번 상위권에 올라간 뒤 계속 유리해질 수 있기 때문이다.
...
{{< figure src="current-ranking-architecture.png" alt="Redis 기반 일별 상품 랭킹 시스템 첫 번째 설계안" class="mx-auto" width="1100" >}}

전체 흐름은 대략 이렇게 이어진다.

1. 사용자가 상품 상세 조회, 좋아요, 주문 결제 같은 행동을 한다.
2. `commerce-api`는 도메인 이벤트를 기록한다.
3. 이벤트는 transaction outbox에 저장된다.
4. outbox relay가 Kafka topic으로 이벤트를 발행한다.
5. `commerce-streamer`가 Kafka 이벤트를 소비한다.
6. 이벤트 종류에 따라 Redis의 일별 ranking key에 metric과 최종 score를 반영한다.
7. 랭킹 API는 Redis에서 rank와 score를 조회하고, MySQL에서 상품 정보를 보강해 응답한다.
...
만약 최종 score 하나만 저장해두면 `viewWeight`나 `likeWeight`를 바꿨을 때 이미 계산된 점수를 다시 해석하기 어렵다. 반면 view, like, sales metric을 분리해두면 현재 가중치를 기준으로 `ranking:all:{date}`를 다시 계산할 수 있다.
...
{{< figure src="rdb-metric-sot-architecture.png" alt="RDB Metric SOT와 Redis Top N 랭킹 아키텍처" class="mx-auto" width="1100" >}}
...
사용자 행동 이벤트는 Kafka로 들어오고, `RankingMetricConsumer`가 이벤트를 소비한다. 다만 consumer가 Redis score를 바로 올리는 대신 RDB의 metric row를 upsert한다.
